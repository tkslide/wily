...
static rdWin *getWin(int user, void *userp, rdWinType kind, char *title, char *filename);
static void clearWin(rdWin *w);
static void freeItems(rdWin *w);
static int loadItems(rdWin *w, char **items, int savecontents);
static int initWin(int user, void *userp, rdWin *w, char *title, char *filename);
...
/*
 * getWin(user, userp, kind, title, filename) - grab a window of the appropriate type.
 */

static rdWin *
getWin(int user, void *userp, rdWinType kind, char *title, char *filename)
{
	rdWin *w;

	if ((!Listwin || kind == rdList && rdMultiList) ||
		(!Artwin || kind == rdArticle && rdMultiArt)) {
...
/*
 * initWin(user, userp, w, title, filename) - initialise the window's tag, label, etc.
 */

static int
initWin(int user, void *userp, rdWin *w, char *title, char *filename)
{
...
/*
 * getArtWin(title, text, filename, savecontents) - get an article window, and load it with
 * either the given text, or from the given file.
 */

int
getArtWin(int user, void *userp, char *title, char *text, char *filename, int savecontents)
{
	rdWin *w = getWin(user, userp, rdArticle, title, filename);
...
{
	rdWin *w = getWin(user, userp, rdList, title, (char *)0);
...
int
itemNumber(rdWin *w, ulong p0, ulong p1, void **ptr)
{
	int n;
	rdItem *i;
...
void
highlightItem(rdWin *w, int n, void *ptr)
{
	static char addr[80];			/* XXX - overkill */
	rdItem *i = (rdItem *)ptr;

	if (i == 0)
		for (i = w->items; n--; i = i->next);
	sprintf(addr,"#%lu,#%lu", i->p0, i->p1);
...
fprintf(stderr,"Hit at (%lu)\n",p0);
		if ((n = itemNumber(w, p0, p0, (void **)&i)) == -1)
			return;		/* sigh */
fprintf(stderr,"Is item %d expands to (%lu,%lu)\n",n, i->p0, i->p1);
		highlightItem(w, n, i);
...
winExec(rdWin *w, char *cmd, char *arg)
{
	...
	*/
	...
	if (w->wintype == rdList) {
		...
		if ((n = itemNumber(w, p0, p1, (void *)&i)) != -1) {
			highlightItem(w, n, i);
			p0 = i->p0;
			p1 = i->p1;
			user_cmdList(w,cmd,p0,p1,n,arg);
			return;
		}
	}
