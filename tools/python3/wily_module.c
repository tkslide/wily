...
static char con_list__doc__[] = "List currently open windows list() Returns list of (name, id) tuples representing currently open windows";
...
static char con_win__doc__[] = "open a window win(name:string, isBackedUp:integer) Returns an integer window identifier, to be used for later operations on the window ";

static PyObject*
wily_win(WilyConnection *self, PyObject *args) {
    char *name;
    int backup;
    Id id;
    char *err;
...
static char con_event__doc__[] = "event() returns an event tuple (w, t, r0, r1, s), whose fields are: w: window identifier t: event type r0, r1: affected range s: string The meaning (if any) of the values of r0, r1 and s depend on the event type 't' ";
...
static char con_eventwouldblock__doc__[] = "eventwouldblock() If eventwouldblock() returns true, calling event() might have to wait.  If eventwouldblock() returns false, calling event() would return immediately because an event is already queued up and waiting";
...
static char con_bounce__doc__[] = "bounce(tuple) Called with an event tuple as returned by event().  Returns None";
...
static char con_attach__doc__[] = "attach(w:integer, mask:integer) 'w' is a window identifier as obtained by new() or list().  'mask' is a bitmask of event types. Sets the mask of events to be sent to us.";
...
static char con_setname__doc__[] = "setname(w:integer, s:string) Set w's name to 's'";
...
static char con_getname__doc__[] = "getname(w:integer) : string Return the name currently visible in w's tag";
...
static char con_settools__doc__[] = "settools(w:integer, s:string) Set w's tools to 's'";
...
static char con_gettools__doc__[] = "gettools(w:integer) : string Return the tools currently visible in w's tag";
...
static char con_replace__doc__[] = "replace(w:integer, from:integer, to:integer, s:string) replace the text in 'w' from 'from' to 'to' with 's'";
...
static char con_run__doc__[] = "run(w:integer, s:string) has the same effect as sweeping 's' with B2  in window 'w'";
...
static PyMethodDef WilyConnection_methods[] = {
    ...
    {"eventwouldblock",    (PyCFunction)wily_eventwouldblock,    METH_VARARGS,  con_eventwouldblock__doc__},
